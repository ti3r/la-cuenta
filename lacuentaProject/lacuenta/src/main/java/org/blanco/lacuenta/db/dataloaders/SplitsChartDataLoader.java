...
public class SplitsChartDataLoader extends AbstractSplitsDataLoader {
    ...
    /***
     * Class that will hold the results of the process that will retrieve the
     * splits from database and extra information needed in order to build the
     * result chart of this class;
     *
     * @author Alexandro Blanco <ti3r.bubblenet@gmail.com>
     *
     */
    ...
    /***
     * Build a List of Split Objects based on the passed load target. It makes
     * use of the getDataCursor(String ) method.
     *
     * @param loadTarget
    ...
    private SplitListBuildResult getSplits(String loadTarget) {
        SplitListBuildResult result = new SplitListBuildResult(
        ...
        result.avg = (result.splits.size() > 0) ? result.expenses
                / result.splits.size() : 0;
        Log.i("La Cuenta", "Splits Char Data Load. Average: " + result.avg);
        Log.i("La Cuenta", "Splits Char Data Load. Expenses: "
                + result.expenses);
        return result;
    }

    /***
     * Prepares the XYMultipleSeriesDataset Object that will be used in the
     * Chart based on the passed Split list.
     *
     * @param splits
     *            A List of Split Objects that will be added to the dataset
    ...
    private XYMultipleSeriesDataset prepareDataSet(List<Split> splits,
        ...
        CategorySeries expensesSeries = new CategorySeries(
                this.activity.getString(R.string.splits));
        CategorySeries averageSeries = new CategorySeries(
                activity.getString(R.string.average));
        CategorySeries tipSeries = new CategorySeries(
                activity.getString(R.string.tips));
        for (Split s : splits) {

            expensesSeries.add(String.valueOf(s.getDate()), s.getResult());
            averageSeries.add(String.valueOf(s.getDate()), avg); // Add the
            // average
            // for each
            // split
            // date
            tipSeries.add(String.valueOf(s.getDate()),
                    (s.getResult() * s.getTip()) / 100);
        }
    ...
    /***
     * It prepares the XYMultipleSeriesRenderer Object that will be used in the
     * chart. It builds the renderer based on hardcoded properties.
     *
     * @return XYMultipleSeriesRenderer Object
     */
    private XYMultipleSeriesRenderer prepareRenderer(int maxValXAxis,
                                                     int maxValYAxis) {
        ...
        mRenderer.setChartTitle(this.activity
                .getString(R.string.expenses_label));
        ...
        prepareXYSeriesRenderer(tipRenderer, Color.YELLOW, PointStyle.SQUARE,
                2f);
     ...
     *
     * @param renderer
     *            the XYSeriesRenderer that the properties will be applied to
     * @param color
     *            the int color that will be applied to the renderer
     * @param style
     *            the PointStyle style that will be applied to the renderer
     * @param width
     *            the float width that will be applied to the renderers line
     */
    ...
    public View doInBackground(String... params) {
        ...
        if (Looper.myLooper() == null)
            Looper.prepare();
        XYMultipleSeriesDataset dataSet = prepareDataSet(splits.splits,
                splits.avg);
        // prepare the renderer with max X and Y Axises values as X=# of splits
        // Records+1
        // and Y=Max Expense of the Splits + 10 %
        XYMultipleSeriesRenderer mRenderer = prepareRenderer(
                splits.splits.size() + 1,
                (int) (splits.maxExpense + (splits.maxExpense * .1)));

        GraphicalView v = ChartFactory.getLineChartView(activity.getActivity(),
                dataSet, mRenderer);
        // ChartFactory.getBarChartView(activity, dataSet, mRenderer,
        // Type.DEFAULT );
        // ChartFactory.getRangeBarChartView(activity, dataSet, mRenderer,
        // Type.DEFAULT);
